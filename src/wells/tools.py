...
function that goes through :mod:`wells.safety` (path confinement +
...
from the agentic executor loop in :mod:`wells.executor`.
...
def _get_index_tools() -> list:
    ...
    try:
        from wells.index_tools import INDEX_TOOLS
...
def _load_skill(ctx: ToolContext, name: str) -> ToolResult:
    """Load a skill's full instructions into context (progressive disclosure).

    The agent sees only each skill's name + one-line description up front (in
    the system prompt's skill index). It calls this tool to pull in the full
    body *only when a request matches that skill*, keeping context focused.
    """
    if not name or not name.strip():
        return ToolResult(False, "", "name is required")
    from wells import skills

    ok, body = skills.load_skill_body(name.strip(), ctx.workspace)
    return ToolResult(ok, body, "" if ok else body)
...
READ_TOOLS: list[ToolDef] = [
    ...
    ToolDef(
        name="load_skill",
        description=(
            "Load a skill's full instructions into your context. Skills are "
            "domain how-tos (e.g. 'how to add a provider', 'how to run the release'). "
            "You see each skill's name + one-line description in the AVAILABLE SKILLS "
            "section of the system prompt — call this ONLY for a skill that matches "
            "the current request, to avoid wasting context. Returns the skill's body."
        ),
        input_schema={
            "type": "object",
            "properties": {
...
# Whether the optional feature tools have been successfully registered. Set to
# True once both codeact + background modules load cleanly. Until then,
# dispatch() retries (handles the circular-import case where background.py is
# imported before tools.py finishes loading).
_optional_registered: bool = False


def _register_optional_tools() -> None:
    """Add feature-gated tools (CodeAct, background agents) to the registry.

    Idempotent and safe to call repeatedly: never duplicates a tool that's
    already registered. Sets the module-level ``_optional_registered`` flag
    only when both modules import cleanly (so a partial import during a circular
    load triggers a retry from :func:`dispatch`).
    """
    global _optional_registered
    existing = {t.name for t in ALL_TOOLS}
    ok = True

    # CodeAct: sandboxed run_code tool.
    try:
        from wells import codeact

        if codeact.enabled() and codeact.RUN_CODE_TOOL.name not in existing:
            ALL_TOOLS.append(codeact.RUN_CODE_TOOL)
            existing.add(codeact.RUN_CODE_TOOL.name)
    except Exception:
        ok = False

    # Background agents: bg_start / bg_status / bg_collect.
    try:
        from wells import background

        if background.enabled():
            for t in (
                background.BG_START_TOOL,
                background.BG_STATUS_TOOL,
                background.BG_COLLECT_TOOL,
            ):
...
def _rebuild_index() -> None:
    """Rebuild the name → ToolDef index from ALL_TOOLS."""
    global _TOOLS_BY_NAME
    _TOOLS_BY_NAME = {t.name: t for t in ALL_TOOLS}


def _ensure_optional_registered() -> None:
    """Retry optional-tool registration if it hasn't succeeded yet.

    Called from :func:`dispatch` so tools resolve even when a circular import
    prevented registration at module-load time.
    """
    if not _optional_registered:
